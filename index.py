...
def download_video(video_id, video_name):
    ...
    audio_url = mpd['MPD']['Period']['AdaptationSet'][1]["Representation"]["BaseURL"]
    ...
    print(mpd['MPD']['Period']['AdaptationSet'][1]["Representation"]["SegmentList"]["SegmentURL"])
    audio += get_segments(
        audio_req,
    ...
    if run_res.returncode:
        err_exit(f"Error video convert invocation: {run_res.stderr.decode('latin-1')}")
    else:
